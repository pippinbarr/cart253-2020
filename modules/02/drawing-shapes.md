...
While learning to program it's a __very__ good idea to follow along and try things out as you learn them. For that reason, we should start a new JavaScript project to play around in for each week's topic. A "sandbox".

1. Download the template project again if you need to (![template-p5-project.zip](https://pippinbarr.github.io/cart253-2020/templates/template-p5-project.zip))
...
- Which can be a challenge because we have to get them __exactly right__
...
- Computer screens are divided up into _pixels_ ("Picture Elements")
...
So the screen is basically just a bunch of little squares, like this:
...
Before we can draw anything, we need to make sure we understand how to say __where__ we want to draw things on a grid like the screen.
...
- The x-axis starts at the __left__ of the screen (or canvas) and points from left to right
- The y-axis starts at the __top__ of the screen (or canvas) and points from top to bottom
